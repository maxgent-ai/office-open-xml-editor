...
| S-STYLE-012 | Chart text and `title` / `axisTitle` shape components | Partial | Numeric/linked/direct text defaults are consumed with atomic text-paint precedence. Across titles, axes, legends, labels, data tables, trendline labels, and display-unit labels, an absent, empty, or metadata-only character-property carrier inherits. A carrier with substantive visual formatting owns the run and resolves an omitted `b`/`i` to regular; an explicit boolean always wins. Numeric title typography is applied only when `c:style` is present because the optional element has no schema default; the Office-observed omitted-style compatibility keeps automatic axis/mark paints but uses a regular-weight title below any direct or chart-space text formatting. For classic dark styles 41–48, automatic text uses the Office-observed theme `lt1` contrast colour across the shared DOCX/XLSX/PPTX parser; a single-textual-paragraph title takes that colour only when its direct `a:pPr/a:defRPr` supplies the observed carrier. Empty rich paragraphs, sibling `c:title/c:txPr`, and unverified multi-paragraph titles do not activate the compatibility rule. Direct and linked text paint still win. Title and axis-title frame fill, outline, and effects are retained only where an existing label-box path exposes them; general classic title-frame shape painting remains unimplemented. |
