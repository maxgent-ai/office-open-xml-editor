...
function makeQuantizedCalibriCanvas(): {
  canvas: HTMLCanvasElement;
  fillTextCalls: FillTextCall[];
} {
  let font = '10px serif';
  ...
  const ctx = {
    ...
    measureText: (s: string) => {
      ...
      return {
        width: [...s].length * p,
        // Chromium exposes whole-CSS-pixel font boxes for the real Calibri
        // face at these sizes: 10px => 10+3, 11px => 10+3. Tight glyph ink is
        // smaller and therefore does not require either box to grow.
        fontBoundingBoxAscent: p === 10 ? 10 : 10,
        fontBoundingBoxDescent: 3,
        actualBoundingBoxAscent: p * 0.64,
        actualBoundingBoxDescent: p * 0.18,
      } as TextMetrics;
    },
    save() {}, restore() {}, beginPath() {}, closePath() {},
    moveTo() {}, lineTo() {}, stroke() {}, fill() {}, fillRect() {},
...
describe('lineRule=auto preserves the authored Calibri design height through substitution', () => {
  it('keeps authored Calibri pagination when the web substitute is unavailable', () => {
    const body = Array.from({ length: 200 }, (_, index) => {
      const p = paragraph(`L${index + 1}`, auto(259 / 240)) as Any;
      p.defaultFontSize = 11;
      p.defaultFontFamily = 'Calibri';
      p.runs[0].fontSize = 11;
      p.runs[0].fontFamily = 'Calibri';
      return p as BodyElement;
    });
    const model = docWith(...body) as Any;
    Object.assign(model.section, {
      pageWidth: 595,
      pageHeight: 842,
      marginTop: 72,
      marginRight: 90,
    ...
    model.fontFamilyClasses = { Calibri: 'swiss' };
    model.majorFont = 'Calibri';
    ...
    expect(layout.pages[0]?.layers.body.filter((node) => node.kind === 'paragraph'))
      .toHaveLength(48);
  });
  ...
  it.each([
    [10, 13],
    [11, 13],
  ])('uses the proven Calibri design metric at %ipt when Canvas quantizes the font box to %ipt', async (fontSize, _fontBox) => {
    const p = paragraph(
      'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAA BBBBBBBBBBBBBBBBBBBBBBBBBBBBBB',
      auto(1),
    ) as Any;
    p.defaultFontSize = fontSize;
    p.defaultFontFamily = 'Calibri';
    p.runs[0].fontSize = fontSize;
    p.runs[0].fontFamily = 'Calibri';
    const model = docWith(p as BodyElement) as Any;
    model.majorFont = 'Calibri';
    model.fontFamilyClasses = { Calibri: 'swiss' };

    const { canvas, fillTextCalls } = makeQuantizedCalibriCanvas();
    await renderDocumentToCanvas(model, canvas, 0, {
      dpr: 1,
      width: 400,
      layoutServices: createLayoutServices(model, {
        useGoogleFonts: true,
        googleFaces: [{
          family: 'Carlito', status: 'loaded', style: 'normal', weight: '400',
        } as unknown as FontFace],
        measureContext: canvas.getContext('2d'),
      }),
    });

    const first = fillTextCalls.find((call) => call.text.startsWith('A'));
    const second = fillTextCalls.find((call) => call.text.startsWith('B'));
    expect(first).toBeDefined();
    expect(second).toBeDefined();
    expect(second!.y - first!.y).toBeCloseTo(fontSize * (2500 / 2048), TOL);
  });

  it('applies w:line=259/240 to Calibri\'s 2500/2048 line box when Carlito is selected', async () => {
    const p = paragraph(
      'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAA BBBBBBBBBBBBBBBBBBBBBBBBBBBBBB',
      auto(259 / 240),
    ) as Any;
    ...
    await renderDocumentToCanvas(model, canvas, 0, {
      ...
      layoutServices: createLayoutServices(model, {
        useGoogleFonts: true,
        // The resolver selects the metric-compatible web substitute. The line
        // box still belongs to the authored Calibri face, not the browser's
        // synthetic 1.0em Canvas box used by this deterministic fixture.
        googleFaces: [{
          family: 'Carlito', status: 'loaded', style: 'normal', weight: '400',
        } as unknown as FontFace],
        measureContext: canvas.getContext('2d'),
    ...
    expect(second!.y - first!.y).toBeCloseTo(
      11 * (2500 / 2048) * (259 / 240),
      TOL,
    );
  });

  it('paginates 200 style-resolved 259/240 Calibri lines at 48 lines on the first A4 page', () => {
    const body = Array.from({ length: 200 }, (_, index) => {
      const p = paragraph(`L${index + 1}`, auto(259 / 240)) as Any;
      p.defaultFontSize = 11;
      p.defaultFontFamily = 'Calibri';
